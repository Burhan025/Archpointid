...
$introductory_title = get_post_meta( get_the_ID(), 'ap_dental_service_solution_intro_title', true );
$introductory_description = get_post_meta( get_the_ID(), 'ap_dental_service_solution_intro_description', true );

//Process Involved section:
$process_section_title = get_post_meta( get_the_ID(), 'ap_dental_service_process_title', true );
$process_group_of_blocks = get_post_meta( get_the_ID(), 'ap_dental_service_process_group_blocks', true );
...
?>
		...
		<main id="main" class="site-main" role="main">
			...
			<section class="archpointSection process">
                ...
                <?php
                // Display the selected custom menu programmatically
                // Get the selected menu ID from custom meta field
                ...
                ?>
					...
					<?php if($process_group_of_blocks) {
						...
						foreach ($process_group_of_blocks as $process_block) {
							...
							$image_id = false;
							if (array_key_exists('ap_dental_service_process_image', $process_group_of_blocks[0])) {
								$image_id = $process_block['ap_dental_service_process_image_id'];
								$image_alt = get_post_meta( $image_id, '_wp_attachment_image_alt', true);
							}				

							/*first item*/
							$first_item_title = false;
							if (array_key_exists('ap_dental_service_process_first_item_title', $process_block)) {
								$first_item_title = $process_block['ap_dental_service_process_first_item_title'];
							}
							$first_item_description = false;
							if (array_key_exists('ap_dental_service_process_first_item_description', $process_block)) {
								$first_item_description = $process_block['ap_dental_service_process_first_item_description'];
							}
							$first_item_see_more_text = false;
							if (array_key_exists('ap_dental_service_process_first_item_see_more_text', $process_block)) {
								$first_item_see_more_text = $process_block['ap_dental_service_process_first_item_see_more_text'];
							}
							$first_item_see_more_url = false;
							if (array_key_exists('ap_dental_service_process_first_item_see_more_url', $process_block)) {
								$first_item_see_more_url = $process_block['ap_dental_service_process_first_item_see_more_url'];
							}
							...
							<div class="row processRow" id="section-<?php echo $counter; ?>">
								...
								<?php if($first_item_title || $first_item_description) { ?>
